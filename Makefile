.PHONY: help install dev-install lint typecheck test clean db-up db-down db-reset pipeline-sp pipeline-all pipeline-incremental dashboard api run-all pre-commit download-sp
...
help:
	...
	@echo "make pipeline-incremental - Executar pipeline incremental (skip dados existentes)"
...
pipeline-incremental:
	PYTHONPATH=$(pwd) python -m src.pipeline_etl --city sao-paulo --incremental
...
api:
	PYTHONPATH=$(pwd) uvicorn src.api.app:app --host 0.0.0.0 --port 8000 --reload
