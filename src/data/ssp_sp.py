...
SPSAFE_URL = "https://zenodo.org/records/16739645/files/SPSafe_CSV.zip"
...
def download_spsafe_dataset(
    ...
    if not zip_path.exists():
        ...
        try:
            resp = requests.get(SPSAFE_URL, timeout=300, stream=True)
            resp.raise_for_status()
            with open(zip_path, "wb") as f:
                for chunk in resp.iter_content(chunk_size=8192):
                    f.write(chunk)
            logger.info("SPSafe baixado: {} bytes", zip_path.stat().st_size)
        except Exception as e:
            logger.warning("Erro ao baixar SPSafe: {}", e)
            return {}
